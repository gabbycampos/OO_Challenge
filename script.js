...
//console.log(myFirstVehicle.toString()) // The vehicle is a Toyota Corolla from 2000.

// PART TWO
class Car extends Vehicle {
  constructor(make, model, year) {
    super(make, model, year);
    this.numWheels = 4; // property with a value of 4.
  }
}
...
class Garage {
  ...
  add(prop) {
    if(!(prop instanceof Vehicle)) {
      return 'Only vehicles are allowed in here!';
    }
    if(this.vehicles.length >= this.capacity) {
      return "Sorry, we're full.";
    }
    this.vehicles.push(prop); // put this at end.
...
console.log(garage.add(new Motorcycle)) // "Sorry, we're full."